...
public class CredentialsManagerTest {
    ...
    public fun shouldSaveRefreshableCredentialsInStorage() {
        ...
        verify(storage).store("cn.authok.id_token", "idToken")
        verify(storage).store("cn.authok.access_token", "accessToken")
        verify(storage).store("cn.authok.refresh_token", "refreshToken")
        verify(storage).store("cn.authok.token_type", "type")
        verify(storage).store("cn.authok.expires_at", expirationTime)
        verify(storage).store("cn.authok.scope", "scope")
        verify(storage).store("cn.authok.cache_expires_at", expirationTime)
    ...
    public fun shouldSaveRefreshableCredentialsUsingAccessTokenExpForCacheExpirationInStorage() {
        ...
        manager.saveCredentials(credentials)
        verify(storage).store("cn.authok.id_token", "")
        verify(storage).store("cn.authok.access_token", "accessToken")
        verify(storage).store("cn.authok.refresh_token", "refreshToken")
        verify(storage).store("cn.authok.token_type", "type")
        verify(storage).store("cn.authok.expires_at", accessTokenExpirationTime)
        verify(storage).store("cn.authok.scope", "scope")
        verify(storage).store("cn.authok.cache_expires_at", accessTokenExpirationTime)
        verifyNoMoreInteractions(storage)
    }
    ...
    public fun shouldSaveRefreshableCredentialsUsingIdTokenExpForCacheExpirationInStorage() {
        ...
        verify(storage).store("cn.authok.expires_at", accessTokenExpirationTime)
        verify(storage).store("cn.authok.scope", "scope")
        verify(storage).store("cn.authok.cache_expires_at", idTokenExpirationTime)
        verifyNoMoreInteractions(storage)
    }
    ...
    public fun shouldSaveNonRefreshableCredentialsInStorage() {
        ...
        verify(storage).store("cn.authok.id_token", "idToken")
        verify(storage).store("cn.authok.access_token", "accessToken")
        verify(storage).store("cn.authok.refresh_token", null as String?)
        verify(storage).store("cn.authok.token_type", "type")
        verify(storage).store("cn.authok.expires_at", expirationTime)
        verify(storage).store("cn.authok.scope", "scope")
        verify(storage).store("cn.authok.cache_expires_at", expirationTime)
    ...
    public fun shouldFailOnGetCredentialsWhenNoAccessTokenOrIdTokenWasSaved() {
        ...
        Mockito.`when`(storage.retrieveString("cn.authok.id_token")).thenReturn(null)
        Mockito.`when`(storage.retrieveString("cn.authok.access_token")).thenReturn(null)
        Mockito.`when`(storage.retrieveString("cn.authok.refresh_token")).thenReturn("refreshToken")
        Mockito.`when`(storage.retrieveString("cn.authok.token_type")).thenReturn("type")
        val expirationTime = CredentialsMock.ONE_HOUR_AHEAD_MS
        Mockito.`when`(storage.retrieveLong("cn.authok.expires_at")).thenReturn(expirationTime)
        Mockito.`when`(storage.retrieveLong("cn.authok.cache_expires_at"))
            .thenReturn(expirationTime)
        Mockito.`when`(storage.retrieveString("cn.authok.scope")).thenReturn("scope")
    ...
    public fun shouldFailOnGetCredentialsWhenExpiredAndNoRefreshTokenWasSaved() {
        verifyNoMoreInteractions(client)
        Mockito.`when`(storage.retrieveString("cn.authok.id_token")).thenReturn("idToken")
        Mockito.`when`(storage.retrieveString("cn.authok.access_token")).thenReturn("accessToken")
        Mockito.`when`(storage.retrieveString("cn.authok.refresh_token")).thenReturn(null)
        Mockito.`when`(storage.retrieveString("cn.authok.token_type")).thenReturn("type")
    ...
    public fun shouldNotFailOnGetCredentialsWhenCacheExpiresAtNotSetButExpiresAtIsPresent() {
        ...
        val expirationTime = CredentialsMock.ONE_HOUR_AHEAD_MS
        Mockito.`when`(storage.retrieveLong("cn.authok.expires_at")).thenReturn(expirationTime)
        Mockito.`when`(storage.retrieveLong("cn.authok.cache_expires_at")).thenReturn(null)
        Mockito.`when`(storage.retrieveString("cn.authok.scope")).thenReturn("scope")
        manager.getCredentials(callback)
    ...
    public fun shouldRenewCredentialsIfSavedScopeIsNullAndRequiredScopeIsNotNull() {
        ...
        Mockito.`when`(storage.retrieveLong("cn.authok.cache_expires_at"))
            .thenReturn(expirationTime)
        Mockito.`when`(storage.retrieveString("cn.authok.scope")).thenReturn(null)
        Mockito.`when`(
        ...
        verify(storage).store("cn.authok.id_token", renewedCredentials.idToken)
        verify(storage).store("cn.authok.access_token", renewedCredentials.accessToken)
        // RefreshToken should not be replaced
        verify(storage, never()).store("cn.authok.refresh_token", newRefresh)
        verify(storage).store("cn.authok.refresh_token", "refreshToken")
        verify(storage).store("cn.authok.token_type", renewedCredentials.type)
        verify(storage).store(
            "cn.authok.expires_at", renewedCredentials.expiresAt.time
        )
        verify(storage).store("cn.authok.scope", renewedCredentials.scope)
        verify(storage).store(
            "cn.authok.cache_expires_at", renewedCredentials.expiresAt.time
        )
    ...
    public fun shouldRenewCredentialsWhenScopeHasChanged() {
        ...
        Mockito.`when`(storage.retrieveLong("cn.authok.cache_expires_at"))
            .thenReturn(expirationTime)
        Mockito.`when`(storage.retrieveString("cn.authok.scope")).thenReturn("some new scope")
        Mockito.`when`(
            client.renewAuth("refreshToken")
        ).thenReturn(request)
        val newDate = Date(CredentialsMock.ONE_HOUR_AHEAD_MS + ONE_HOUR_SECONDS * 1000)
    ...
    public fun shouldGetAndFailToRenewExpiredCredentialsWhenReceivedTokenHasLowerTtl() {
        ...
        MatcherAssert.assertThat(
            exception.message,
            Is.`is`("The lifetime of the renewed Access Token (1) is less than the minTTL requested (60). Increase the 'Token Expiration' setting of your Authok API in the dashboard, or request a lower minTTL.")
        )
    }
    ...
    public fun shouldGetAndSuccessfullyRenewExpiredCredentialsWithRefreshTokenRotation() {
        ...
        verify(storage).store("cn.authok.refresh_token", "rotatedRefreshToken")
        verify(storage).store("cn.authok.token_type", renewedCredentials.type)
        verify(storage).store(
            "cn.authok.expires_at", renewedCredentials.expiresAt.time
        )
    ...
    public fun shouldClearCredentials() {
        ...
        verify(storage).remove("cn.authok.id_token")
        verify(storage).remove("cn.authok.access_token")
        verify(storage).remove("cn.authok.refresh_token")
        verify(storage).remove("cn.authok.token_type")
        verify(storage).remove("cn.authok.expires_at")
        verify(storage).remove("cn.authok.scope")
        verify(storage).remove("cn.authok.cache_expires_at")
        verifyNoMoreInteractions(storage)
    }
    ...
    public fun shouldAddParametersToRequest() {
        ...
        Mockito.`when`(storage.retrieveLong("cn.authok.cache_expires_at"))
            .thenReturn(expirationTime)
        Mockito.`when`(storage.retrieveString("cn.authok.scope")).thenReturn("oldscope")
        Mockito.`when`(
            client.renewAuth("refreshToken")
        ).thenReturn(request)
