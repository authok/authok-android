...
public class SecureCredentialsManagerTest {
    ...
    public fun shouldCreateAManagerInstance() {
        val context: Context =
            Robolectric.buildActivity(Activity::class.java).create().start().resume().get()
        val apiClient = AuthenticationAPIClient(Authok("clientId", "domain"))
    ...
    public fun shouldSaveRefreshableCredentialsInStorage() {
        ...
        verify(storage)
            .store(eq("cn.authok.credentials"), stringCaptor.capture())
        verify(storage).store("cn.authok.credentials_expires_at", sharedExpirationTime)
        verify(storage)
            .store("cn.authok.credentials_access_token_expires_at", sharedExpirationTime)
        verify(storage).store("cn.authok.credentials_can_refresh", true)
    ...
    public fun shouldSaveRefreshableCredentialsUsingAccessTokenExpForCacheExpirationInStorage() {
        ...
        verify(storage)
            .store(eq("cn.authok.credentials"), stringCaptor.capture())
        verify(storage).store("cn.authok.credentials_expires_at", accessTokenExpirationTime)
        verify(storage)
            .store("cn.authok.credentials_access_token_expires_at", accessTokenExpirationTime)
        verify(storage).store("cn.authok.credentials_can_refresh", true)
        verifyNoMoreInteractions(storage)
        val encodedJson = stringCaptor.firstValue
        MatcherAssert.assertThat(encodedJson, Is.`is`(Matchers.notNullValue()))
    ...
    public fun shouldSaveRefreshableCredentialsUsingIdTokenExpForCacheExpirationInStorage() {
        ...
        verify(storage)
            .store(eq("cn.authok.credentials"), stringCaptor.capture())
        verify(storage).store("cn.authok.credentials_expires_at", idTokenExpirationTime)
        verify(storage)
            .store("cn.authok.credentials_access_token_expires_at", accessTokenExpirationTime)
        verify(storage).store("cn.authok.credentials_can_refresh", true)
        verifyNoMoreInteractions(storage)
    ...
    public fun shouldSaveNonRefreshableCredentialsInStorage() {
        ...
        verify(storage)
            .store(eq("cn.authok.credentials"), stringCaptor.capture())
        verify(storage).store("cn.authok.credentials_expires_at", expirationTime)
        verify(storage)
            .store("cn.authok.credentials_access_token_expires_at", expirationTime)
        verify(storage).store("cn.authok.credentials_can_refresh", false)
        verifyNoMoreInteractions(storage)
        val encodedJson = stringCaptor.firstValue
        MatcherAssert.assertThat(encodedJson, Is.`is`(Matchers.notNullValue()))
        val decoded = Base64.decode(encodedJson, Base64.DEFAULT)
    ...
    public fun shouldClearStoredCredentialsAndThrowOnSaveOnCryptoException() {
        ...
        verify(storage).remove("cn.authok.credentials")
        verify(storage).remove("cn.authok.credentials_expires_at")
        verify(storage).remove("cn.authok.credentials_can_refresh")
    }
    ...
    public fun shouldFailOnGetCredentialsWhenIncompatibleDeviceExceptionIsThrown() {
        ...
        verify(storage, never()).remove("cn.authok.credentials")
        verify(storage, never()).remove("cn.authok.credentials_expires_at")
        verify(storage, never()).remove("cn.authok.credentials_can_refresh")
    }
    ...
    public fun shouldRenewCredentialsWithMinTtl() {
        ...
        verify(storage)
            .store(eq("cn.authok.credentials"), stringCaptor.capture())
        verify(storage).store("cn.authok.credentials_expires_at", newDate.time)
        verify(storage).store("cn.authok.credentials_access_token_expires_at", newDate.time)
        verify(storage).store("cn.authok.credentials_can_refresh", true)
    ...
    public fun shouldGetAndFailToRenewExpiredCredentialsWhenReceivedTokenHasLowerTtl() {
        ...
        verify(storage, never())
            .store(eq("cn.authok.credentials"), anyString())
        verify(storage, never()).store(
            eq("cn.authok.credentials_expires_at"),
            anyLong()
        )
        verify(storage, never()).store(
            eq("cn.authok.credentials_can_refresh"),
            anyBoolean()
        )
        verify(storage, never()).remove(anyString())
    }
    ...
    public fun shouldNotHaveCredentialsWhenAccessTokenWillExpireAndNoRefreshTokenIsAvailable() {
        val expirationTime = CredentialsMock.ONE_HOUR_AHEAD_MS
        Mockito.`when`(storage.retrieveLong("cn.authok.credentials_expires_at"))
            .thenReturn(expirationTime)
        Mockito.`when`(storage.retrieveLong("cn.authok.credentials_access_token_expires_at"))
            .thenReturn(expirationTime)
        Mockito.`when`(storage.retrieveBoolean("cn.authok.credentials_can_refresh"))
            .thenReturn(false)
        Mockito.`when`(storage.retrieveString("cn.authok.credentials"))
...
//        requestCallbackCaptor.firstValue.onSuccess(renewedCredentials)TODO poovam
    ...
    public fun shouldClearCredentials() {
        ...
        verify(storage).remove("cn.authok.credentials")
        verify(storage).remove("cn.authok.credentials_expires_at")
        verify(storage).remove("cn.authok.credentials_access_token_expires_at")
        verify(storage).remove("cn.authok.credentials_can_refresh")
        verifyNoMoreInteractions(storage)
    }
    ...
    private fun insertTestCredentials(
        ...
        Mockito.`when`(storage.retrieveString("cn.authok.credentials")).thenReturn(encoded)
        Mockito.`when`(storage.retrieveLong("cn.authok.credentials_expires_at")).thenReturn(
            willExpireAt.time
        )
        Mockito.`when`(storage.retrieveBoolean("cn.authok.credentials_can_refresh"))
    ...
    private companion object {
        ...
        private const val KEY_ALIAS = "cn.authok.key"
    }
}