...
import cn.authok.android.provider.WebAuthProvider.resume
import cn.authok.android.request.DefaultClient
import cn.authok.android.request.HttpMethod.POST
...
import cn.authok.android.util.SSLTestUtils
...
public class WebAuthProviderTest {
    ...
    public fun setUp() {
        ...
        account = Authok(JwtTestUtils.EXPECTED_AUDIENCE, JwtTestUtils.EXPECTED_BASE_DOMAIN)
    ...
    public fun shouldSetAudienceFromParametersOnLogin() {
        val parameters =
            Collections.singletonMap("audience", "https://mydomain.authok.cn/myapi" as Any)
        login(account)
        ...
        assertThat(
            uri,
            UriMatchers.hasParamWithValue("audience", "https://mydomain.authok.cn/myapi")
        )
    }
    ...
    public fun shouldSetOrganizationAndInvitationFromInvitationUrl() {
        login(account)
            .withInvitationUrl("https://tenant.authok.cn/login?organization=travel0&invitation=inv123")
    ...
    public fun shouldFailWhenInvitationUrlDoesNotContainOrganization() {
        login(account)
            .withInvitationUrl("https://tenant.authok.cn/login?invitation=inv123")
            .start(activity, callback)
    ...
    public fun shouldFailWhenInvitationUrlDoesNotContainInvitation() {
        login(account)
            .withInvitationUrl("https://tenant.authok.cn/login?organization=travel0")
            .start(activity, callback)
    ...
    public fun shouldHaveRedirectUriOnLogin() {
        ...
        assertThat(
            ...
            `is`("https://test.domain.com/android/cn.authok.android.authok.test/callback")
        )
    }
    ...
    public fun shouldResumeLoginWithIntentWithCodeGrant() {
        ...
        val proxyAccount: Authok = Authok(JwtTestUtils.EXPECTED_AUDIENCE, mockAPI.domain)
    ...
    public fun shouldResumeLoginWithCustomNetworkingClient() {
        ...
        val proxyAccount = Authok(JwtTestUtils.EXPECTED_AUDIENCE, JwtTestUtils.EXPECTED_BASE_DOMAIN)
        ...
        assertThat<Map<String, String>>(
            codeOptionsCaptor.firstValue.headers, Matchers.`is`(
                IsMapContaining.hasKey("Authok-Client")
            )
        )
    }
    ...
    public fun shouldFailToResumeLoginWithIntentWithConfigurationInvalid() {
        ...
        assertThat(
            authExceptionCaptor.firstValue.getDescription(),
            `is`("The application isn't configured properly for the social connection. Please check your Authok's application configuration")
        )
    }
    ...
    public fun shouldHaveDefaultSchemeOnLogout() {
        ...
        val uri =
            intentCaptor.firstValue.getParcelableExtra<Uri>(AuthenticationActivity.EXTRA_AUTHORIZE_URI)
        assertThat(uri, `is`(notNullValue()))
        assertThat(uri, UriMatchers.hasParamWithName("return_to"))
        val returnToUri = Uri.parse(uri?.getQueryParameter("return_to"))
    ...
    public fun shouldHaveReturnToUriOnLogout() {
        ...
        assertThat(uri, `is`(notNullValue()))
        assertThat(
            uri?.getQueryParameter("return_to"),
            `is`("https://test.domain.com/android/cn.authok.android.authok.test/callback")
        )
    }
    ...
    private fun createAuthIntent(hash: String?): Intent {
        ...
        val validUri = Uri.parse("https://domain.authok.cn/android/package/callback$hash")
