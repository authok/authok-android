...
import cn.authok.android.request.internal.OidcUtils
...
internal class OAuthManager(
        ...
        pkce!!.getToken(
            ...
            object : Callback<Credentials, AuthenticationException> {
                override fun onSuccess(credentials: Credentials) {
                    assertValidIdToken(
                        credentials.idToken,
                        object : Callback<Void?, AuthokException> {
                            override fun onSuccess(result: Void?) {
                                callback.onSuccess(credentials)
                            }

                            override fun onFailure(error: AuthokException) {
                ...
                override fun onFailure(error: AuthenticationException) {
                    if ("Unauthorized" == error.getDescription()) {
                        Log.e(
                            PKCE.TAG,
                            "Unable to complete authentication with PKCE. PKCE support can be enabled by setting Application Type to 'Native' and Token Endpoint Authentication Method to 'None' for this app at 'https://mgmt.authok.cn/#/applications/" + apiClient.clientId + "/settings'."
                        )
                    }
    ...
    private fun assertValidIdToken(
        idToken: String?,
        validationCallback: Callback<Void?, AuthokException>
    ) {
    ...
    private fun assertNoError(errorValue: String?, errorDescription: String?) {
        ...
        Log.e(
            TAG,
            "Error, access denied. Check that the required Permissions are granted and that the Application has this Connection configured in Authok Dashboard."
        )
        when {
            ...
            ERROR_VALUE_LOGIN_REQUIRED == errorValue -> {
                ...
                throw AuthenticationException(
                    ...
                    "The application isn't configured properly for the social connection. Please check your Authok's application configuration"
                )
            }
        }
    }
    ...
    private fun addClientParameters(parameters: MutableMap<String, String>, redirectUri: String) {
        parameters[KEY_AUTHOK_CLIENT_INFO] = account.authokUserAgent.value
    ...
    companion object {
        ...
        private const val KEY_AUTHOK_CLIENT_INFO = "authokClient"
