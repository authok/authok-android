...
public class CredentialsManager @VisibleForTesting(otherwise = VisibleForTesting.PRIVATE) internal constructor(
     ...
     *
     * @param authenticationClient the Authok Authentication client to refresh credentials with.
            ...
            try {
                ...
                if (willAccessTokenExpire) {
                    ...
                    val wrongTtlException = CredentialsManagerException(
                        String.format(
                            Locale.getDefault(),
                            "The lifetime of the renewed Access Token (%d) is less than the minTTL requested (%d). Increase the 'Token Expiration' setting of your Authok API in the dashboard, or request a lower minTTL.",
                            tokenLifetime,
                            minTtl
                        )
                    )
    ...
    private companion object {
        private const val KEY_ACCESS_TOKEN = "cn.authok.access_token"
        private const val KEY_REFRESH_TOKEN = "cn.authok.refresh_token"
        private const val KEY_ID_TOKEN = "cn.authok.id_token"
        private const val KEY_TOKEN_TYPE = "cn.authok.token_type"
        private const val KEY_EXPIRES_AT = "cn.authok.expires_at"
        private const val KEY_SCOPE = "cn.authok.scope"
        private const val KEY_CACHE_EXPIRES_AT = "cn.authok.cache_expires_at"
    }
}