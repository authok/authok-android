...
import cn.authok.android.NetworkErrorException

public class AuthenticationException : AuthokException {
    ...
    public constructor(message: String, cause: AuthokException? = null) : super(message, cause)
    ...
    private fun warnIfOIDCError() {
        if ("invalid_request" == getCode() && (ERROR_OIDC_ACCESS_TOKEN == getDescription() || ERROR_OIDC_RO == getDescription())) {
            Log.w(
                ...
                "Your Authok Application is configured as 'OIDC Conformant' but this instance it's not. To authenticate you will need to enable the flag by calling Authok#setOIDCConformant(true) on the Authok instance you used in the setup."
            )
        }
    }

    /**
     * Authok error code if the server returned one or an internal library code (e.g.: when the server could not be reached)
     *
    ...
    /// When Authok rule returns an error. The message returned by the rule will be in `description`
    ...
    /// When authenticating with web-based authentication using prompt=none and the authok session had expired
